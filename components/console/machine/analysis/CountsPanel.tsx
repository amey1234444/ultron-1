...
export function CountsPanel({ counts }: { counts: AnalysisCounts }) {
  ...
  return (
    <View className="gap-3">
      <Text className={cn('font-body-medium text-[12.5px] uppercase tracking-wider', mutedClass)}>Counts · this machine</Text>

      <View className="flex-row items-center gap-2 pb-1" style={{ borderBottomWidth: 1, borderBottomColor: hairline }}>
        <Text className={cn('flex-1 font-mono text-[9.5px] uppercase tracking-wider', mutedClass)}>Kind</Text>
        <Text style={{ width: 54 }} className={cn('text-right font-mono text-[9.5px] uppercase tracking-wider', mutedClass)}>Findings</Text>
        <Text style={{ width: 42 }} className={cn('text-right font-mono text-[9.5px] uppercase tracking-wider', mutedClass)}>Rules</Text>
      </View>
      ...
      {SEVERITY_ORDER.map((severity) => (
        <View key={severity} className="flex-row items-center gap-2">
          ...
          <Text style={{ width: 54, color: severityHex[severity] }} className="text-right font-mono text-[14.5px] font-bold tabular-nums">
            {counts.findingsBySeverity[severity]}
          </Text>
          <Text style={{ width: 42 }} className={cn('text-right font-mono text-[12.5px] tabular-nums', mutedClass)}>
      ...
      <View className="gap-1.5 pt-1">
        <Text className={cn('font-mono text-[9.5px] uppercase tracking-wider', mutedClass)}>Severity mix · by count</Text>
        {total === 0 ? (
          <Text className={cn('font-body text-[11.5px] italic', mutedClass)}>Nothing has fired.</Text>
        ) : (
