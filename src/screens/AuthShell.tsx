...
export function AuthShell({ subtitle, children }: { subtitle: string; children: ReactNode }) {
  ...
  return (
    <View className="flex-1 items-center justify-center px-6 py-12" style={{ backgroundColor: '#0A0A0A' }}>
      ...
      <View
        className="w-full max-w-sm overflow-hidden rounded-3xl border p-8"
      ...
      >
        <LinearGradient
          ...
          style={{ height: 3, width: 46, borderRadius: 999, marginBottom: 20 }}
        />
        <Text style={{ fontFamily: AUTH_FONT_DISPLAY, fontSize: 44, letterSpacing: 7, color: '#F5F5F5', lineHeight: 46 }}>ULTRON</Text>
        <Text style={{ fontFamily: AUTH_FONT_BODY, fontSize: 14, color: '#8A8A8A', marginTop: 6 }}>{subtitle}</Text>
  ...
  return (
    <View className="mt-4 gap-1.5">
      ...
      <TextInput
        ...
        className="rounded-xl border border-line-dark px-4 py-3 text-base text-ink"
        {...props}
      />
    </View>
  );
...
export function AuthButton({ label, submitting, onPress }: { label: string; submitting?: boolean; onPress: () => void }) {
  return (
    <Pressable
      ...
      className={`mt-7 items-center rounded-xl px-4 py-3.5 ${submitting ? 'opacity-60' : ''}`}
